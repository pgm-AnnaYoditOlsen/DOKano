document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateCanoeAvailability = async () => {
        ...
        if (!selectedDate || !selectedTime) {
            canoeAvailabilitySection.innerHTML = '<p class="text-center font-bold text-l">Selecteer aub een datum en een tijdstip</p>';
            return;
        }
        ...
        if (availability !== null) {
            if (availability.error) {
                ...
                for (const [canoeType, remainingCanoes] of Object.entries(availability)) {
                    ...
                    const canoeElement = document.querySelector(`[data-canoe-type="${canoeType}"]:not(.hidden)`);
                    if (canoeElement) {
    ...
    nextButton.addEventListener('click', (event) => {
        ...
        if (checkCanoeAvailabilityBeforeNext() = false) {
